...
Use this checklist when you want to generate clips in a UI-based video tool and keep the rest of the pipeline automated.
...
2. Copy the `first_frame_prompt` for scene 1 into your image or video tool.
3. Generate the first frame and save it as `output/scenes/scene_01_first_frame.png`.
4. Copy the matching `video_prompt`, `negative_prompt`, `duration_seconds`, and `transition_to_next`.
