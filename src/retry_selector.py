...
def build_retry_selection(render_status: Dict[str, Any], retry_plan: Dict[str, Any]) -> Dict[str, Any]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Select retry scenes from render status and retry plan.")
    parser.add_argument("render_status_json")
    parser.add_argument("retry_plan_json")
    parser.add_argument("--output", default="-")
    args = parser.parse_args()

    selection = build_retry_selection(load_json(args.render_status_json), load_json(args.retry_plan_json))
