 ...
 */
export const INCLUDED_SOURCE_REVISION_KEYS = new Set([
...
export function isPlanStale(project, currentDraft = null) {
    if (!project || !project.source_revision) return true;
    if (project.is_stale) return true;
    if (currentDraft) {
        try {
            const currentRev = computeSourceRevisionSync(currentDraft);
            if (currentRev !== project.source_revision) return true;
        } catch (_) {
            return true;
        }
    }
...
export function performCopyAction(project, action, sceneId = null, currentDraft = null) {
    if (isPlanStale(project, currentDraft)) {
...
export function redactSecrets(text) {
    ...
    let redacted = text.replace(/nvapi-[A-Za-z0-9_-]{10,}/g, '[REDACTED]');
    redacted = redacted.replace(/Bearer\s+[A-Za-z0-9._-]+/gi, 'Bearer [REDACTED]');
    redacted = redacted.replace(/(?:api_key|nim_api_key|secret|token)\s*=\s*["'][^"']+["']/gi, '$1="[REDACTED]"');
    return redacted;
}
    ...
    const sanitize = (target) => {
        ...
        if (target && typeof target === 'object') {
            for (const key of Object.keys(target)) {
                ...
                if (['api_key', 'nim_api_key', 'secret', 'token', 'authorization'].includes(keyLower)) {
                    delete target[key];
                } else if (typeof target[key] === 'string') {
                    target[key] = redactSecrets(target[key]);
...
export function loadProjectState() {
    ...
    try {
        const raw = JSON.parse(stored).project;
        return redactObjectSecrets(raw);
    } catch {
        return null;
    }
}
